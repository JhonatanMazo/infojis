...
from app import db
from app.models import Asignacion, User, Curso, Asignatura, Actividad
...
def index():
    ...
    active_config = get_active_config()
    anio_lectivo = active_config['anio'] if active_config and 'anio' in active_config else None
    ...
    # Filtro de cursos según el rol
    if current_user.rol == 'docente':
        cursos = (
            db.session.query(Curso)
            .join(Asignacion, Asignacion.id_curso == Curso.id)
            .filter(
