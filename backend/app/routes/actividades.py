...
def index():
    ...
    if hasattr(current_user, 'rol') and current_user.rol == 'docente':
        # Para docentes, obtener los IDs de sus asignaciones
        asignaciones_docente_ids = [asig.id for asig in Asignacion.query.filter_by(id_docente=current_user.id).all()]
        # Filtrar actividades que pertenecen a sus asignaciones
        query = query.filter(Actividad.id_asignacion.in_(asignaciones_docente_ids))
    
    # Todos los usuarios (incluidos docentes) no ven las actividades que ellos mismos crearon
