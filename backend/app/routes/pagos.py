...
from app import db
from app.models import Curso, Matricula, Pago, Asignacion
...
def listar_pagos():
    ...
    if current_user.is_admin():
        ...
        cursos_ids = db.session.query(Matricula.id_curso).filter(
            Matricula.año_lectivo == anio_lectivo, 
            Matricula.estado == 'activo'
        ).distinct().all() if anio_lectivo else []
...
def enviar_comprobante(id):
    ...
    # Configurar email
    msg = MIMEMultipart()
    msg['Subject'] = f'Comprobante de Pago - {pago.concepto} - Jardín Infantil Sonrisas'
    msg['From'] = current_app.config['MAIL_USERNAME']
    msg['To'] = matricula.email
    ...
    # Adjuntar PDF premium
    pdf_adjunto = MIMEApplication(pdf_data, _subtype="pdf")
    pdf_adjunto.add_header('Content-Disposition', 'attachment', filename=f'comprobante_pago_{id}.pdf')
